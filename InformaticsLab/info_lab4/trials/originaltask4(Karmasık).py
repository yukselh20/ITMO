def loads(string):
    """Parse JSON string into Python data structure manually."""
    string = string.strip()
    if string.startswith('{'):
        return parse_object(string[1:].strip())
    elif string.startswith('['):
        return parse_array(string[1:].strip())
    raise ValueError("Invalid JSON input")

def parse_object(string):
    """Parse a JSON object (dictionary)."""
    obj = {}
    while string:
        string = string.lstrip()
        if string.startswith('}'): 
            return obj, string[1:].strip()
        key, string = parse_string(string)
        string = string.lstrip()
        if not string.startswith(':'):
            raise ValueError("Expected ':' after key in object")
        value, string = parse_value(string[1:].strip())
        obj[key] = value
        string = string.lstrip()
        if string.startswith('}'):
            return obj, string[1:].strip()
        elif string.startswith(','):
            string = string[1:]
        else:
            raise ValueError("Expected ',' or '}' in object")
    raise ValueError("Unterminated object")

def parse_array(string):
    """Parse a JSON array (list)."""
    arr = []
    while string:
        string = string.lstrip()
        if string.startswith(']'):
            return arr, string[1:].strip()
        value, string = parse_value(string)
        arr.append(value)
        string = string.lstrip()
        if string.startswith(']'):
            return arr, string[1:].strip()
        elif string.startswith(','):
            string = string[1:]
        else:
            raise ValueError("Expected ',' or ']' in array")
    raise ValueError("Unterminated array")

def parse_string(string):
    """Parse a JSON string."""
    end_index = string.find('"', 1)
    if end_index == -1:
        raise ValueError("Unterminated string")
    return string[1:end_index], string[end_index + 1:].strip()

def parse_number(string):
    """Parse a JSON number (int or float)."""
    num_str = ''
    for char in string:
        if char in '0123456789.-+eE':
            num_str += char
        else:
            break
    if '.' in num_str or 'e' in num_str or 'E' in num_str:
        return float(num_str), string[len(num_str):].strip()
    else:
        return int(num_str), string[len(num_str):].strip()

def parse_value(string):
    """Parse any JSON value."""
    if string.startswith('"'):
        return parse_string(string)
    elif string.startswith('{'):
        return parse_object(string[1:].strip())
    elif string.startswith('['):
        return parse_array(string[1:].strip())
    elif string.startswith('true'):
        return True, string[4:].strip()
    elif string.startswith('false'):
        return False, string[5:].strip()
    elif string.startswith('null'):
        return None, string[4:].strip()
    else:
        return parse_number(string)

def json_to_csv(data):
    """Convert parsed JSON data to CSV format."""
    # CSV header
    csv_lines = ["day,even_week,group_name,class_id,title,class_format,type,campus,time,auditory,teacher"]
    
    # Process schedule data
    even_week = str(data.get('even_week', '')).lower()
    group_name = data.get('group_name', '')
    days = data.get('days', {})
    
    for day, day_data in days.items():
        schedule = day_data.get('schedule', [])
        for class_info in schedule:
            # Escape any commas in fields
            fields = [
                day,
                even_week,
                group_name,
                str(class_info.get('id', '')),
                class_info.get('title', '').replace(',', ';'),
                class_info.get('class_format', '').replace(',', ';'),
                class_info.get('type', '').replace(',', ';'),
                class_info.get('campus', '').replace(',', ';'),
                class_info.get('time', ''),
                class_info.get('auditory', '').replace(',', ';'),
                class_info.get('teacher', '').replace(',', ';')
            ]
            # Add quotes if field contains commas or quotes
            fields = [f'"{field}"' if (',' in field or '"' in field) else field for field in fields]
            csv_lines.append(','.join(fields))
    
    return '\n'.join(csv_lines)

if __name__ == "__main__":
    input_file = "in.json"
    output_file = "outTask4.csv"

    # Read JSON input
    with open(input_file, "r", encoding="utf-8") as f:
        json_content = f.read()

    # Parse JSON and convert to CSV
    parsed_json, _ = loads(json_content)
    csv_content = json_to_csv(parsed_json)

    # Write CSV output
    with open(output_file, "w", encoding="utf-8") as f:
        f.write(csv_content)

    print("Conversion complete!")